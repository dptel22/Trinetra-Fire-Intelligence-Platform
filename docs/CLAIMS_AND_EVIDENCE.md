...
| C-06 | `unclassified` is an optional post-training abstention label (env `UNCLASSIFIED_THRESHOLD`), **disabled by default**. | model | `app/core/config.py:154-158` (default `None`); `app/services/model_service.py:308-314` | VERIFIED | "Optional abstention, off by default." Never call it a trained class. |
...
| C-26 | Offline basemap: NASA Blue Marble raster tiles (z0–z6) ship in `frontend/public/tiles/bluemarble/` and work today; the OpenMapTiles **PMTiles archive is not present** — Streets/Topographic styles degrade to a flat fallback with an explicit "not installed" notice until `docs/PMTILES_BUILD.md` is run. | frontend | Directory listing (0 `*.pmtiles` files, 2026-09-10); `frontend/src/services/basemapStyles.js` (`PMTILES_AVAILABLE`); `FireMapPage.jsx` fallback notice | VERIFIED | "Works offline on the shipped Blue Marble basemap; the vector PMTiles pack is an optional local build." |
...
| C-33 | "NTRO compliance", "defense-grade explainability", "national-scale/real-time alerting", "zero-downtime", "100% secure", "fully automated", "production-ready": **no repository evidence for any of these.** | governance | Adversarial grep 2026-09-10: hits exist only in superseded research essays (`docs/Final model.md`, `docs/Beyond SNPP-Only…md`, `docs/demo-script.md`) | UNSUPPORTED | Never use in docs or pitch. The honest differentiator is the built-in honesty/provenance machinery (C-07, C-27, C-19), not adjectives. |
| C-34 | Historical docs claiming XGBoost, H3 res 7, risk tiers (`high_risk`/`medium_risk`/`low_risk`/`no_fire`), six classes, 52 features, Postgres/Redis/WebSocket, AUC 0.87, "<1 ms inference", "ML worker retrains daily": all describe removed systems or never-built plans. | history | `docs/demo-script.md` (deprecated banner), `docs/architecture.md` (superseded — being rewritten), `docs/decisions/0001-model-choice.md` (superseded banner), `docs/eda-findings.md` (early exploration) | HISTORICAL | Only inside documents explicitly bannered as historical/superseded. |
...
| Source | Why |
|---|---|
| `docs/demo-script.md` (body) | Describes the removed XGBoost/Postgres demo; metrics (AUC 0.87, F1 0.72, <1 ms) were never reproducible in the current repo. |
...
| `docs/eda-findings.md` | Early exploration: risk-tier taxonomy and res-7 recommendation superseded; area figures partially wrong. |
| `docs/decisions/0001-model-choice.md` | Bannered superseded (XGBoost era). |
...
| `docs/Final model.md`, `docs/Beyond SNPP-Only…md`, and the research PDFs | Rationale essays; assume 6 classes; "NTRO compliance"/"defense-grade" framing is project interpretation, not an official requirement. |
| `docs/backend-rebuild-coordination.md` (body) | Historical coordination log; only its final reconciliation appendix matched current truth, now superseded by `docs/CURRENT_PROJECT_TRUTH.md`. |
