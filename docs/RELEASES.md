...
| **Serving parquets** (FIRMS + OSM/WRI features) | GitHub Release `serving-data-2026-09-09` | Pinned snapshot for fresh clones (`python scripts/fetch_serving_data.py`). Measured 2026-09-20: 813,789 cell-days, 463,127 cells, acq 2024-08-01 → 2026-09-08, detections in 10 states (6 training + 4 blind-test). Ships `SHA256SUMS.json` in `{file, Hash}` (PowerShell `Get-FileHash`) shape; the fetch script accepts that and `{name, sha256}`. **Refresh pending**: a nationwide archive backfill is in progress (2026-09-20, see `docs/superpowers/plans/2026-09-20-nationwide-archive-backfill.md` and `AGENT_LOG.md`); a new `serving-data-YYYY-MM-DD` release will be cut only after the rebuild, seasonal gate, and live-tail merge are validated. Until then the 09-09 snapshot does not reflect the latest docs: figures in `CURRENT_PROJECT_TRUTH.md`, `CLAIMS_AND_EVIDENCE.md` (C-18), `architecture.md` and `WHOLE_SYSTEM_AUDIT.md` that cite 459,972 cells / 20 states/UTs / data through 2026-09-10 describe the unreleased local refresh, not this release. |
