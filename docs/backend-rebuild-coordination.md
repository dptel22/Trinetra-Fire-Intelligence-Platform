...
### Viewport Predictions Contract: Zoom, Mode & Cap Semantics (2026-09-08)

The `GET /predictions` (and `/api/v1/predictions`) endpoint returns `ViewportPredictionsResponse` containing `{ mode, zoom, total_predictions, predictions: PredictionResponse[] }`. All predictions are strictly returned at native H3 resolution 8. The query parameter `zoom` (default 8.0, range 1–20) and response field `mode` (`"aggregated_macro"` when latitude or longitude span exceeds 20.0 degrees, else `"detailed_hexagons"`) are declared informative and decorative metadata; the backend does not downsample or coarse-grain H3 hexagons server-side. Furthermore, DuckDB enforces a hard ceiling of `LIMIT 2500` rows per viewport query (`feature_store.py:149`). Client applications (specifically the React MapLibre frontend in `api.js`) must perform 2×2 recursive bounding box tiling when querying broad geographical expanses to prevent truncating hotspot detections exceeding 2,500 cells.
