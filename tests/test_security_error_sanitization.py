...
def test_classify_hotspot_error_sanitization(caplog):
...
def test_classify_batch_error_sanitization(caplog):
    with patch("app.api.endpoints.classify.model_service.predict_single") as mock_predict:
        mock_predict.side_effect = RuntimeError(SENSITIVE_LEAK_STRING)
        with caplog.at_level(logging.ERROR):
...
def test_explain_hotspot_error_sanitization(caplog):
...
def test_predictions_viewport_error_sanitization(caplog):
...
def test_cell_explanation_error_sanitization(caplog):
...
def test_cell_detail_error_sanitization(caplog):
    with patch("app.api.endpoints.classify.model_service.get_cell_detail") as mock_cell:
        mock_cell.side_effect = RuntimeError(SENSITIVE_LEAK_STRING)
        params = {"acq_date": "2024-05-15"}
        with caplog.at_level(logging.ERROR):
...
def test_audit_override_error_sanitization(caplog):
...
def test_audit_logs_error_sanitization(caplog):
